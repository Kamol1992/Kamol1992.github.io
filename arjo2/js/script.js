...
const array = [];
const listLP = document.getElementById('LP');
...
buttonAcepted.addEventListener('click', () => {
    ...
    const indexNumber = Math.floor(parseFloat(numberProduct)/10);
    lp = lp + 1;
    /*HTML code creation*/
    const tekstHtml = td.innerHTML = `<tr class="order"><td id="table--lp">${lp}</td><td id="table--number-product">${lastQuantity} x ${numberProduct}</td><td id="clock-${lp}" class="table-time">-- --</td><td id="table-status--${lp}">Zrealizuj</td><td><div class="table-button--acepted" id="button-${lp}" title="Zrealizuj"><i class="far fa-check-circle"></i></div></div>
    <div class="table-button--deleted" id="button-delete--${lp}"><i class="far fa-times-circle"></i></div></td></td></tr>`;
    ...
    td.style.order = indexNumber;
        ...
        /* lead time */
            ...
            listLP.addEventListener('click', () =>{
                td.style.order = 'unset';
            })
...
console.log(index);
        ...
        const tableHeader = document.querySelector('.table-list-order');
        ...
        listLP.addEventListener('click', ()=>{
            console.log('klikkLP');
            tableHeader.classList.toggle('table-header--active');
            tableBody.classList.toggle('table-body--active');
